...
find_colorpoints = function(words, dims){
  ...
  colordf <- data.frame(words$connectingNodes, i_to_rc(words$connectingNodes, dims), rep(3, length(words$connectingNodes)), rep("blue", length(words$connectingNodes)))
  colnames(colordf) <- c("point", "row", "col", "colorIndex", "color")
  
  #Add terminal YELLOW nodes
  orange_point <- data.frame(words$terminalNodes, i_to_rc(words$terminalNodes, dims), rep(6, length(words$terminalNodes)), rep("orange", length(words$terminalNodes)))
  ...
  if (length(words$terminalNodes) != 0){
    ...
    green_point <- data.frame(beginning_node, i_to_rc(beginning_node, dims), 1, "green")
    ...
    red_point <- data.frame(ending_node, i_to_rc(ending_node, dims), 2, "red")
    colnames(red_point) <- c("point", "row", "col", "colorIndex", "color")
    colordf <- rbind(colordf, red_point)
  }

  #Add Node at highest GREEN point in total path
  rcs = i_to_rci(words$wordPath, dims)
  highest_point_index = rcs[[which.min(rcs[ ,1]), 3]]
  purple_point <- data.frame(highest_point_index, i_to_rc(highest_point_index, dims), 4, "purple")
